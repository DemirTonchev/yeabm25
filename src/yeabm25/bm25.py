...
class YeaBM25:
    """
    The class representing the Okapi-BM25 variant of the algorithm. Okapi-BM25 is a ranking
    function widely used as the OLD(before transformers went
    brrrrrrrrrrrrr) state of the art for Information Retrieval tasks. BM25 is a variation
      of the TF-IDF model.
    This implementation applies negative idf correction from here:
        https://arxiv.org/pdf/1602.03606.pdf#page=4. Which also sets a floor a floor on the
    ...
    doc_freqs : list[dict[str, int]]
        Word Frequency per document. [{'cat': 1}, {'foo': 5}] means  the first document contains
          the term 'hi' 1 time and
    ...
    doc_len : list[int]
        Number of terms per document. So [3, 6] means the first document contains 3 terms and
        second 6 terms.
    ...
    def __init__(self, k1: float = 1.5, b: float = 0.75, epsilon: float = 0.25):
        ...
        # word -> number of documents with word {'cat': 50} means 'cat' is in 50
        # documents fromthe corpus
    ...
    def _process_doc(self, document: list[str]) -> None:
        ...
        Appends frequencies of words per document and builds the word document frequencies. Wodf
        df is used for idf.
        """
    ...
    def _calc_idf(self):
        ...
        for word, freq in self.word_df.items():
            ...
            # if idf is negative we set as 0, for correct running mean calculation.
            # This is not explicit in the linked paper
            ...
            if idf_value > 0:
                ...
                self.average_idf = self.average_idf + (idf_value - self.average_idf) / (
                    len(self.idf)
                )
    ...
    def fit(self, corpus: list[list[str]] | list[BMDocument]):
        """
        Fits the index given a corpus of documents. In the case of bm25 a document is list of
        words(strings)
        """
    ...
    def document_vector(self, idx):
        ...
        return [scores[f] if f in scores else 0.0 for f in feats]
